LorisPartialsBinData {
	...
	*new { arg name, parentPath;	
		^super.newCopyArgs(name, parentPath).init;
	}
	...
	*dir_ {arg path; if(path.last == $/, { dir = path; }, { dir = path ++ $/ });}
	...
	init {
		...
		dataDict = Object.readBinaryArchive(thisDir ++ name ++ "-data");
		dataDict.notNil.if({
	...
	*build { arg sdifFile, name, defDir, fade = true;
		var list, times, phases, dataDict, dur = 0;
		name = name.asString;		
